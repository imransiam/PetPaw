...
const ThemeControl = () => {
  ...
  // 2. This function runs every time the 'theme' state changes
  useEffect(() => {
    localStorage.setItem("theme", theme);
    const localTheme = localStorage.getItem("theme");
    
    // This line actually switches the theme for the whole website
    document.querySelector("html").setAttribute("data-theme", localTheme);
  }, [theme]);

  // 3. Handle the toggle switch
  ...
  {/* sun icon (shows when checked is false) */}
  ...
  {/* moon icon (shows when checked is true) */}
